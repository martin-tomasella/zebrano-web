...
export default function ProspectoDetalle() {
  ...
  const enviarMensaje = async () => {
    ...
    setEnviando(true);
    await supabase.from('prospecto_conversaciones').insert({ prospecto_id:id, canal:msgCanal, direccion:msgDir, contenido:nuevoMsg.trim(), tipo:'mensaje' });
  ...
  const guardarCampo = async (campo, valor) => {
    await supabase.from('prospectos').update({[campo]:valor}).eq('id',id);
...
function ModalCompra({ prospecto_id, onClose, onSave }) {
  ...
  const guardar = async () => {
    ...
    setSaving(true);
    await supabase.from('historial_compras').insert({ prospecto_id, ...form, monto_cotizado:form.monto_cotizado?parseFloat(form.monto_cotizado):null, monto_final:form.monto_final?parseFloat(form.monto_final):null });
