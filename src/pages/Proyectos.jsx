...
export default function Proyectos() {
  ...
  useEffect(() => {
    const load = async () => {
      const { data, error } = await supabase.from('proyectos').select('*, clientes(nombre)').order('created_at',{ascending:false});
              ...
              <tbody>
                {proyectos.map(p => (
                  <tr key={p.id}>
                    ...
                    <td style={{color:'var(--z-text-2)'}}>{p.clientes?.nombre||'—'}</td>
                    ...
                    <td style={{color:'var(--z-success)'}}>{p.valor_final?`$${Number(p.valor_final).toLocaleString('es-AR')}`:p.valor_estimado?`$${Number(p.valor_estimado).toLocaleString('es-AR')}`:'—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
