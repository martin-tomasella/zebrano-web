...
const NAV = [
  ...
  { section: 'Ventas' },
  { path: '/prospectos',    label: 'Prospectos',      icon: 'funnel' },
...
function NavIcon({ type, active }) {
  ...
  const icons = {
    ...
    funnel: <svg width="13" height="13" viewBox="0 0 14 14" fill="none"><path d="M1 2h12l-4 5v4l-4-2V7L1 2z" stroke={c} strokeWidth="1" strokeLinejoin="round"/></svg>,
...
export function Layout({ children }) {
  ...
  return (
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', background:'#0A0D08' }}>
      <aside style={{ width:200, minWidth:200, background:'#080B06', borderRight:'1px solid rgba(74,107,54,0.12)', display:'flex', flexDirection:'column', height:'100vh' }}>
        ...
        <nav style={{ flex:1, overflowY:'auto', padding:'6px 0' }}>
          {NAV.map((item, i) => {
            ...
            if (item.section) return (
              <div key={i} style={{ padding:'12px 18px 4px', fontSize:9, color:'#1E3014', textTransform:'uppercase', letterSpacing:'0.14em' }}>{item.section}</div>
            )
            ...
            return (
              <div key={item.path} onClick={() => navigate(item.path)}
                style={{ display:'flex', alignItems:'center', gap:9, padding:isSubItem?'5px 18px 5px 30px':'7px 18px', cursor:'pointer', fontSize:isSubItem?11:12, fontWeight:300, letterSpacing:'0.02em', color:active?'#C8D9B8':isSubItem?'#2A3E24':'#3A5030', background:active?'rgba(74,107,54,0.08)':'transparent', borderLeft:active?'2px solid #4A6B36':'2px solid transparent', transition:'all 0.12s' }}
                onMouseEnter={e=>{ if(!active){e.currentTarget.style.background='rgba(74,107,54,0.04)';e.currentTarget.style.color='#8A9E82'}}}
                onMouseLeave={e=>{ if(!active){e.currentTarget.style.background='transparent';e.currentTarget.style.color=isSubItem?'#2A3E24':'#3A5030'}}}>
        ...
        <div style={{ padding:'12px 16px', borderTop:'1px solid rgba(74,107,54,0.1)', display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontSize:11, color:'#C8D9B8', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{profile?.nombre || 'Administrador'}</div>
            <div style={{ fontSize:9, color:'#2E4A22', textTransform:'uppercase', letterSpacing:'0.08em' }}>{profile?.rol || 'admin'}</div>
          </div>
          <button onClick={signOut} title="Cerrar sesión" style={{ background:'none', border:'none', color:'#2E4A22', cursor:'pointer', fontSize:14, lineHeight:1, padding:2 }}
            onMouseEnter={e=>e.target.style.color='#8A9E82'} onMouseLeave={e=>e.target.style.color='#2E4A22'}>⏻</button>
        </div>
      </aside>
...
export function Topbar({ title, subtitle, actions }) {
  return (
    <div style={{ padding:'14px 24px', borderBottom:'1px solid rgba(74,107,54,0.1)', background:'#080B06', display:'flex', alignItems:'center', justifyContent:'space-between', minHeight:54 }}>
      <div>
...
export function PageContent({ children, pad = 24 }) {
  return <div style={{ flex:1, overflowY:'auto', padding:pad, background:'#0A0D08' }}>{children}</div>
}
